...
trait LindaDSL { self: SpatialTuples =>
  ...
  def rd(tupleTemplate: SituatedTupleTemplate): TupleOpId = TupleOpId(s"${mid()}_rd_$T")(tupleTemplate.situation match {
  ...
  def in(tupleTemplate: SituatedTupleTemplate): TupleOpId = TupleOpId(s"${mid()}_in_$T")(tupleTemplate.situation match {
  ...
  implicit class RichTupleOpId(toid: TupleOpId) {
    def prepend(s: String): TupleOpId = toid.copy(uid = s"${s}_${toid.uid}")(toid.op, alchemistTimestamp.toDouble, timeout = toid.timeout)
  }
